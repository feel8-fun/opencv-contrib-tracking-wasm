...
const useDenseOpticalFlow = async () => {
    const cv = await cvLoadWASM();
    const algo = new cv.DISOpticalFlow(cv.DISOPTICAL_FLOW_PRESET_MEDIUM);
    const flow = new cv.Mat();
    const flowXY = new cv.MatVector();
    ...
    const setupCanvas = () => {
        ...
        canvas.width = videoWidth / 8
        canvas.height = videoHeight / 8
    ...
    const vizHSV = new cv.Mat();
    const vizFlowMagnitude = new cv.Mat();
    const vizNormalizedMagnitude = new cv.Mat();
    const vizFlowAngle = new cv.Mat();
    const vizColor = new cv.Mat();

    const vizFlow = () => {
        // Draw Dense flow in HSV
        cv.cartToPolar(flowXY.get(0), flowXY.get(1), vizFlowMagnitude, vizFlowAngle);

        // Normalize magnitude for HSV Value channel (0-1 range)
        cv.normalize(vizFlowMagnitude, vizNormalizedMagnitude, 0, 1, cv.NORM_MINMAX, cv.CV_32F);

        // Create saturation channel (full saturation where there's flow)
        const saturation = new cv.Mat(vizFlowMagnitude.rows, vizFlowMagnitude.cols, cv.CV_32F, new cv.Scalar(0.0));

        const vizHSVChannels = new cv.MatVector();
        vizHSVChannels.push_back(vizFlowAngle);      // Hue channel
        vizHSVChannels.push_back(saturation);         // Saturation channel (full)
        vizHSVChannels.push_back(vizNormalizedMagnitude); // Value channel (normalized)
        cv.merge(vizHSVChannels, vizHSV);

        cv.cvtColor(vizHSV, vizColor, cv.COLOR_HSV2RGBA);
        cv.imshow(outputElement, vizColor);
    ...
    const computeOpticalFlow = () => {
        ...
        const curIdx = frameCount % 2;
        cv.cvtColor(src, dualGray[curIdx], cv.COLOR_RGBA2GRAY)

        if (frameCount > 0) {
            const prevIdx = (curIdx + 1) % 2;
            // Calculate dense optical flow
            algo.calc(dualGray[prevIdx], dualGray[curIdx], flow);
            cv.split(flow, flowXY);
            const mx = medianFromMat(flowXY.get(0));
            const my = medianFromMat(flowXY.get(1));
    ...
    const startProcessing = (input, output, fps) => {
        inputElement = input
    ...
    const medianFromMat = (mat) => {
        ...
        // Accumulate values for CDF
        for (let i = 1; i < histSize[0]; i++) {
            let prevValue = cdf.data32F[i - 1];
            cdf.data32F[i] += prevValue;
        }

        const middle_point = cdf.data32F[histSize[0] - 1] / 2;
        let medianBin;
        for (let i = 0; i < histSize[0]; i++) {
            if (cdf.data32F[i] >= middle_point) {
                medianBin = i;
                break;
            }
        }
        ...
        const medianVal = medianBin / histSize[0] * (ranges[1] - ranges[0]) + ranges[0];
    ...
    return { inputElement, outputElement, ctx, startProcessing, stopProcessing, cleanup, setPerformanceCallback }
}
...
const handlePlay = () => {
    ...
    if (showOpticalFlow.value) {
        processor.startProcessing(videoElement.value, outputCanvas.value, targetFPS.value)
    }
}
const handlePause = () => {
...
onMounted(async () => {
    ...
    processor = await useDenseOpticalFlow();
